<template>
  <div :class="s.dataTable" v-loading="loading">
    <template v-if="!loading && data.length">
        ...
        <el-col :span="spanList[0]">
          <div :class="s.nameItem">
            <div :class="s.nameMain">
              ...
              <quality
                v-if="item.quality && (item.quality['320'] || item.quality['999'])"
                :sq="item.quality['999']"
                :class="s.quality"
              ></quality>
            </div>
            <div :class="s.songControl" v-if="showOperate">
              <slot name="songControlPrepend" :row="item" :$index="limit * (page - 1) + index"></slot>
      ...
      <el-pagination
        v-if="pagination && !loading && data.length > limit"
