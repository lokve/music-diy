...
const track = (id) => {
    ...
    return Promise.all(
        [
            ...
            .slice(
                headers.cookie || typeof window !== 'undefined' ? (select.ENABLE_FLAC ? 0 : 1) : 2,
            )
...
const searchList = (info) => {
    ...
    return request('GET', url)
        .then((response) => response.jsonp())
        .then((jsonBody) => {
            console.log(jsonBody.data.song.list);
            return jsonBody.data.song.list.map((item) => ({
                id: 'qq' + item.id,
                songId: item.id,
                name: item.name,
                artists: item.singer,
                album: Object.assign(item.album, {
                    cover: `https://y.gtimg.cn/music/photo_new/T002R300x300M000${item.album.mid}.jpg`,
                }),
                vendor: 'qq',
                newVendor: 'qq',
                duration: item.interval * 1000,
                param: {song: item.mid, file: item.file.media_mid},
            }));
        });
};
...
module.exports = {check, track, searchList, getSong};
