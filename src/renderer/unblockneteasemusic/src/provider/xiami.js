...
const track = (id) => {
    const url =
        'https://api.xiami.com/web?v=2.0&app_key=1' + '&id=' + id + '&callback=jsonp&r=song/detail';
...
const searchList = (info) => {
    ...
    return request('GET', url, headers)
        .then((response) => response.jsonp())
        .then((jsonBody) => {
            const list = jsonBody.data.songs;
            console.log(list);
            return list.map((item) => ({
                id: 'xiami' + item.song_id,
                songId: item.song_id,
                name: item.song_name,
                artists: [{id: item.artist_id, name: item.artist_name}],
