        ...
        >
            ...
            <div :class="s.songs">
                <div
                    ...
                    :class="{
                 [s.item]: true,
                 [s.active]: info.id === item.id && info.vendor === item.vendor
                 }"
                    @click="play({info: item})"
                >
                    <div :class="s.album_wrap">
                        <img :class="s.album" :src="item.album.cover | image(item.vendor, 70)" />
                    </div>
                    <div :class="s.main">
                        <span :class="s.name">{{item.name}}</span>
                        <span :class="s.singer">{{item.artists.map(item => item.name).join(' ')}}</span>
                        <div
                            :class="s.volumeAnimation"
                            v-if="info.id === item.id && info.vendor === item.vendor"
                        >
...
.slideLeft_enter_active,
.slideLeft_leave_active {
            ...
            .main {
                ...
                .volumeAnimation {
                    ...
                    & > div {
                        ...
                        &.line2 {
                            animation: line 0.6s 0.2s infinite ease-in-out
                                alternate;
                        }
                        &.line3 {
                            animation: line 0.6s 0.4s infinite ease-in-out
                                alternate;
                        }
                        &.line4 {
                            animation: line 0.6s 0.6s infinite ease-in-out
                                alternate;
                        }
                        @keyframes line {
                            from {
