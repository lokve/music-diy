        ...
        methods: {
                    ...
                    if (data.status) {
                        ...
                        for (let index = 0; index < this.album.songs.length; index++) {
                            ...
                            if (tsong.cp) {
                                ...
                                if (data.status) {
                                    const songs = data.data.filter(item => item.id && !item.cp && item.vendor !== tsong.vendor).map(item => {
                                        return {
                                            ...item,
                                            songId: item.id
                                        }
            ...
            async importSongs() {
                ...
                try {
                    if (this.offline) {
                        // TODO
                        localStorage.setItem(this.getOfflineStoreName(), JSON.stringify(this.album.songs));
